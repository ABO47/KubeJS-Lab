...
public final class ModalHeader {
    ...
    public static ButtonWidget closeButton(int x, Runnable onClose) {
        ResourceTexture icon = IconAtlas.iconTexture("close", UiColors.ERROR);
        ...
        button.setHoverTexture((g, mx, my, x0, y0, w0, h0) ->
                UiGlow.drawGlow(g, mx, my, (int) x0, (int) y0, (int) w0, (int) h0));
        return button;
    }

    public static WidgetGroup titleLabel(String title, int left, int width) {
        return new WidgetGroup(left, TITLE_Y, width, TITLE_H) {
            private final TextTexture tex = new TextTexture(title, UiColors.TEXT_PRIMARY)
