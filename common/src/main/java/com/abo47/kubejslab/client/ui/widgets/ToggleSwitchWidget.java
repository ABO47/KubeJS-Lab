...
public final class ToggleSwitchWidget extends SwitchWidget {
    ...
    private static final ColorRectTexture[] BORDER_TEX = new ColorRectTexture[]{
            new ColorRectTexture(UiColors.BORDER_BASE),
            new ColorRectTexture(UiColors.SUCCESS)};
    private static final ColorRectTexture TRACK_TEX =
            new ColorRectTexture(UiColors.withAlpha(UiColors.SURFACE_BASE, 210));
    private static final ColorRectTexture[] KNOB_TEX = new ColorRectTexture[]{
            new ColorRectTexture(UiColors.TEXT_SECONDARY),
            new ColorRectTexture(UiColors.TEXT_PRIMARY)};
    private static final ColorRectTexture SHADOW_TEX =
            new ColorRectTexture(UiColors.withAlpha(UiColors.SURFACE_BASE, 100));
    private static final ColorRectTexture[] HIGHLIGHT_TEX = new ColorRectTexture[]{
            new ColorRectTexture(UiColors.withAlpha(UiColors.TEXT_PRIMARY, 48)),
            new ColorRectTexture(UiColors.withAlpha(UiColors.TEXT_PRIMARY, 90))};
    private static final ColorRectTexture[] ACTIVE_TRACK_TEX = new ColorRectTexture[79];

    static {
        for (int i = 0; i < ACTIVE_TRACK_TEX.length; i++) {
            ACTIVE_TRACK_TEX[i] = new ColorRectTexture(UiColors.withAlpha(UiColors.SUCCESS, 70 + i));
        }
    }
    ...
    public ToggleSwitchWidget(int x, int y, BooleanSupplier valueSupplier, Consumer<Boolean> valueConsumer, Runnable refresh) {
