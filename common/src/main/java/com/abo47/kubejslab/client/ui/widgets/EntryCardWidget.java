...
public class EntryCardWidget extends Widget {
    ...
    protected static final IGuiTexture SELECTED_FILL =
            new ColorRectTexture(UiColors.withAlpha(UiColors.INTERACTIVE, 30));
    ...
    public EntryCardWidget(int x, int y, int w, int h, ItemStack icon, String name, String idLine,
            Runnable onClick, CardRightClick onRightClick) {
        super(x, y, w, h);
        this.iconTex = new ItemStackTexture(icon);
        this.textX = ICON_X + ICON_SIZE + TEXT_GAP;
        this.textW = w - textX;
        this.nameTex = new TextTexture(name, UiColors.TEXT_PRIMARY)
                .setWidth(textW)
                .setType(TextTexture.TextType.LEFT_HIDE);
        this.idTex = new TextTexture(idLine, UiColors.TEXT_MUTED)
                .setWidth(textW)
                .setType(TextTexture.TextType.LEFT_HIDE);
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        if (button == UiColors.MOUSE_BUTTON_RIGHT) {
