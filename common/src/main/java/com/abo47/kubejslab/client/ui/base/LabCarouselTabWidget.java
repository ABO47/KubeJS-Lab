...
public final class LabCarouselTabWidget extends Widget {
    ...
    private static final int CENTER_COLOR = LabColors.TEXT_PRIMARY;
    private static final int SIDE_COLOR = LabColors.withAlpha(LabColors.TEXT_MUTED, SIDE_ALPHA);
    private static final int CHEVRON_COLOR = LabColors.TEXT_SECONDARY;
    private static final int CHEVRON_HOVER_COLOR = LabColors.TEXT_PRIMARY;
    ...
    public LabCarouselTabWidget(int x, int y, int w, int h, String[] keys, int initialSelected) {
        super(x, y, w, h);
        setClientSideWidget();
        this.keys = keys.clone();
        this.selected = Math.max(0, Math.min(initialSelected, keys.length - 1));
        this.leftIcon = LabIconAtlas.iconTexture("back", CHEVRON_COLOR);
        this.rightIcon = LabIconAtlas.iconTexture("forward", CHEVRON_COLOR);
        this.leftIconHover = LabIconAtlas.iconTexture("back", CHEVRON_HOVER_COLOR);
        this.rightIconHover = LabIconAtlas.iconTexture("forward", CHEVRON_HOVER_COLOR);
    ...
    private void ensureIcons() {
        if (leftIcon == null) leftIcon = LabIconAtlas.iconTexture("back", CHEVRON_COLOR);
        if (rightIcon == null) rightIcon = LabIconAtlas.iconTexture("forward", CHEVRON_COLOR);
        if (leftIconHover == null) leftIconHover = LabIconAtlas.iconTexture("back", CHEVRON_HOVER_COLOR);
        if (rightIconHover == null) rightIconHover = LabIconAtlas.iconTexture("forward", CHEVRON_HOVER_COLOR);
    }
    ...
    public void drawInBackground(@Nonnull GuiGraphics g, int mx, int my, float pt) {
        ...
        if ((hoverLeft || hoverPrev) && isMouseOverElement(mx, my)) {
            ...
            LabGlow.drawGlow(g, mx, my, glowX, glowY, glowW, glowH);
        }
        ...
        if (keys.length > 1) {
            ...
            drawScaledText(g, prevComp, prevX, prevY, hoverPrev || hoverLeft ? LabColors.TEXT_SECONDARY : SIDE_COLOR, SIDE_SCALE);
            ...
            drawScaledText(g, nextComp, nextX, nextY, hoverNext || hoverRight ? LabColors.TEXT_SECONDARY : SIDE_COLOR, SIDE_SCALE);
        }
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        // allow clicking in left/right halves as fallback for carousel feel
        int x = getPositionX();
        int w = getSizeWidth();
