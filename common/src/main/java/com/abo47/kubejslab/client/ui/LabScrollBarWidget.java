...
public final class LabScrollBarWidget extends Widget {
    ...
    private static final ColorRectTexture RAIL_TEX = new ColorRectTexture(
            LabColors.withAlpha(LabColors.SURFACE_PANEL_ALT, 170));
    private static final ColorRectTexture THUMB_TEX = new ColorRectTexture(LabColors.BORDER_BASE);
    private static final ColorRectTexture THUMB_ACTIVE_TEX = new ColorRectTexture(LabColors.TEXT_MUTED);
    ...
    public boolean mouseWheelMove(double mouseX, double mouseY, double wheelDelta) {
        ...
        int next = LabScrollMath.wheel(current, max, step, wheelDelta);
        if (next != current) {
    ...
    private boolean updateFromMouse(double mouseY) {
        ...
        int next = LabScrollMath.byMouse(
