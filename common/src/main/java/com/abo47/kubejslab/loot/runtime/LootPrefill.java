...
public final class LootPrefill {
    ...
    public static LootFieldValues prefill(MinecraftServer server, ResourceLocation id, String lootType) {
        ...
        List<LootPoolValues> poolValues = new ArrayList<>();
        ...
        return new LootFieldValues(id.toString(), "", poolValues,
                Math.max(0, pools.size() - poolCount), droppedEntries);
    }

    public static LootFieldValues blankFor(ResourceLocation id) {
        LootFieldValues defaults = LootFieldValues.defaults();
        return new LootFieldValues(id.toString(), defaults.customId(), defaults.pools(), 0, 0);
    }

    public static ResourceLocation lootTableId(ResourceLocation id, String lootType) {
        if (LootService.LOOT_TYPE_BLOCK.equals(lootType)) {
    ...
    private static LootPoolValues parsePool(JsonObject pool, int[] droppedOut) {
        ...
        List<LootEntryValues> allEntries = new ArrayList<>();
        ...
        List<LootEntryValues> entryValues =
    ...
    private static LootEntryValues withGroup(LootEntryValues entry, int group) {
        return new LootEntryValues(entry.type(), entry.item(), entry.tag(), entry.lootTable(),
            ...
            case "minecraft:furnace_smelt" -> {
                if (function.has("conditions") && function.get("conditions").isJsonArray()) {
                    for (JsonElement el : function.getAsJsonArray("conditions")) {
                        if (el.isJsonObject()
                                && LootNotes.WHILE_BURNING.equals(entityNote(el.getAsJsonObject()))) {
                            return LootNotes.SMELTED_BURNING;
                        }
                    }
                }
                return LootNotes.SMELTED;
            }
            case "minecraft:copy_state" -> {
                return LootNotes.COPY_STATE;
            }
            case "minecraft:copy_name" -> {
                return LootNotes.COPY_NAME;
            }
            case "minecraft:set_contents" -> {
                return LootNotes.KEEP_CONTENTS;
            }
            case "minecraft:enchant_randomly" -> {
                return LootNotes.ENCHANT_RANDOM;
            }
            case "minecraft:enchant_with_levels" -> {
                return LootNotes.ENCHANTED;
            }
            case "minecraft:fill_player_head" -> {
                return LootNotes.PLAYER_HEAD;
            }
            case "minecraft:set_nbt", "minecraft:set_name", "minecraft:set_lore", "minecraft:copy_nbt",
                    "minecraft:set_attributes" -> {
                return LootNotes.NBT_EDITS;
            }
            default -> {
    ...
    private static String describeCondition(JsonObject condition) {
        ...
        return switch (name) {
            case "minecraft:killed_by_player" -> LootNotes.KILLED_BY_PLAYER;
            case "minecraft:survives_explosion" -> LootNotes.SURVIVES_EXPLOSION;
            case "minecraft:random_chance" -> LootNotes.encode(LootNotes.CHANCE,
                    formatPercent(asFloat(condition.get("chance"), 1f)));
            case "minecraft:random_chance_with_looting" -> LootNotes.encode(LootNotes.CHANCE_LOOTING,
                    formatPercent(asFloat(condition.get("chance"), 0f)),
                    formatPercent(asFloat(condition.get("looting_multiplier"), 0f)));
            case "minecraft:match_tool" -> LootNotes.encode(LootNotes.MATCH_TOOL, toolNote(condition));
            ...
            case "minecraft:inverted" -> condition.has("term") && condition.get("term").isJsonObject()
                    ? LootNotes.encode(LootNotes.INVERTED,
                            describeCondition(condition.getAsJsonObject("term")))
                    : "inverted";
            case "minecraft:alternative", "minecraft:any_of" ->
                LootNotes.encode(LootNotes.ANY_OF, joinTerms(condition).toArray(new String[0]));
            case "minecraft:all_of" ->
                LootNotes.encode(LootNotes.ALL_OF, joinTerms(condition).toArray(new String[0]));
    ...
    private static String toolNote(JsonObject condition) {
        if (!condition.has("predicate") || !condition.get("predicate").isJsonObject()) {
            return LootNotes.MATCHING_TOOL;
        }
        JsonObject predicate = condition.getAsJsonObject("predicate");
        if (!predicate.has("enchantments") || !predicate.get("enchantments").isJsonArray()) {
            return LootNotes.MATCHING_TOOL;
        ...
        for (JsonElement el : predicate.getAsJsonArray("enchantments")) {
            ...
            String ref = LootNotes.vanillaRef("enchantment." + id);
            if (enchant.has("levels")) {
                float[] levels = rangeOf(enchant.get("levels"), 1f);
                parts.add(levels[0] == levels[1]
                        ? LootNotes.encode(LootNotes.ENCHANT, ref)
                        : LootNotes.encode(LootNotes.ENCHANT_LEVELED, ref,
                                formatCount(levels[0]) + "-" + formatCount(levels[1])));
            } else {
                parts.add(LootNotes.encode(LootNotes.ENCHANT, ref));
            }
        }
        if (parts.isEmpty()) {
            return LootNotes.MATCHING_TOOL;
        ...
        return parts.size() == 1 ? parts.get(0)
                : LootNotes.encode(LootNotes.ANY_OF, parts.toArray(new String[0]));
    }
    ...
    private static String entityNote(JsonObject condition) {
        if (condition.has("predicate") && condition.get("predicate").isJsonObject()) {
            ...
            if (predicate.has("flags") && predicate.getAsJsonObject("flags").has("is_on_fire")
                ...
                return LootNotes.WHILE_BURNING;
            }
        }
